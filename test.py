...
def compute_max_gap(combo, masks, threshold, exact=False):
    """
    計算最大相鄰命中期距：
      threshold: 下限命中數，exact=False 表示 matches>=threshold，exact=True 表示 matches==threshold
    """
    m = 0
    for n in combo:
        m |= 1 << (n - 1)
...
def main(sheet_range, combo_size, file_path):
    ...
    try:
        ...
        print("範圍格式錯誤"); sys.exit(1)
    sm, em = re.match(r"([A-Za-z]+)(\d+)?", sc), re.match(r"([A-Za-z]+)(\d+)?", ec)
    if not sm or not em:
        print("解析範圍失敗"); sys.exit(1)
    sr = int(sm.group(2) or 1); er = int(em.group(2) or ws.max_row)
    c1 = column_index_from_string(sm.group(1)); c2 = column_index_from_string(em.group(1))
    ...
    # 篩選並排序：依照 2星／3星／恰4星：先過濾 max_gap_limit，再取前 top_n
    cat1 = [itm for itm in cat1
            if compute_max_gap(itm[0], masks, threshold=2, exact=False) <= max_gap_limit][:top_n]
    cat2 = [itm for itm in cat2
            if compute_max_gap(itm[0], masks, threshold=3, exact=False) <= max_gap_limit][:top_n]
    cat3 = [itm for itm in cat3
            if compute_max_gap(itm[0], masks, threshold=4, exact=True)  <= max_gap_limit][:top_n]
...
if __name__ == '__main__':
    freeze_support()
    if len(sys.argv)<4:
        print("用法：<SheetRange> <combo_size> <excel_path>")
        sys.exit(1)
